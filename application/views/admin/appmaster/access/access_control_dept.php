...
$script = <<< "JS"
	...
	function showAccessControlDept(deptId) {
        var selectedRank;
        var selectedMenu;
        var selectedAcc;
        ...
        var mLayout = mainTab.cells("access_control_dept_" + deptId).attachLayout({
            pattern: "4T",
            cells: [{
                    id: "a",
                    text: "Jabatan",
        ...
        async function treesRank() {
            const treeRank = await reqJsonResponse(AccessDept("getRanks"), "GET", null);
        ...
        async function treesMenu(rankId) {
            if(rankId) {
                const treeMenu = await reqJsonResponse(AccessDept("getMenus", {deptId, rankId: selectedRank}), "GET", null);
                ...
                treeMenuView.attachEvent("onCheck", function(id, state){
                    let menuId = id.replace("menu-", "");
                    reqJson(AccessDept("updateMenus"), "POST", {
                        deptId,
                        rankId: selectedRank,
                        menuId,
        ...
        async function treesAccordion(menuId) {
            if(menuId) {
                const treeAccordion = await reqJsonResponse(AccessDept("getAccordions", {deptId, rankId: selectedRank, menuId}), "GET", null);
                ...
                treeAccordionView.attachEvent("onCheck", function (code, state) {
                    if(!inactiveAcc) {
                        reqJson(AccessDept("updateAccordions"), "POST", {
                            deptId,
                            rankId: selectedRank,
                            menuId: selectedMenu,
                            accCode: code,
        ...
        function treesAccTree(accCode) {
            if(accCode) {
                reqJson(AccessDept("getTrees", {deptId, rankId: selectedRank, menuId: selectedMenu, accCode}), "GET", null, (err, res) => {
                    var items;
                    if(res.status === "success") {
                        items = res.items;
                    } else {
                ...
                case "save":
                    if(selectedAcc) {
                        if(treeAccordionView.isItemChecked(selectedAcc)) {
                            reqJson(AccessDept("updateTrees"), "POST", {
                                deptId,
                                rankId: selectedRank,
                                menuId: selectedMenu,
                                accCode: selectedAcc,
